...
require_once(
    _PS_MODULE_DIR_.'lgseoredirect'.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.'LGJsonApi.php'
);
require_once(
    _PS_MODULE_DIR_.'lgseoredirect'.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.'LGSeoRedirectPubli.php'
);
...
class LGSEORedirect extends Module
{
    ...
    protected $pnf_pagination               = array();
    ...
    protected $pages_not_found              = array();
    ...
    public function __construct()
    {
        ...
        $this->pnf_pagination          = array(2, 5,10,100,500,1000);
        $this->pnf_total_pages         = 0;
        $this->pnf_total_redirects     = 0;
        $this->pnf_filters             = Tools::getValue('filters_pnf', array());
        ...
        // página estábamos visualizando para dejarlo en la misma, tomando como referencia el primer producto de
    ...
    public function getContent()
    {
        ...
        $this->pages_not_found = $this->getPagesNotFound(
            (($this->pnf_offset - 1) * $this->pnf_limit),
            $this->pnf_limit
        );
    ...
    {
        $response     = array();
        $redirects    = Tools::getValue('redirects', array());
    ...
    {
        $response        = array();
        $pages_not_found = Tools::getValue('pages_not_found', array());
    ...
    protected function getRedirects($limit = 0, $offset = 100)
    {
            ...
            foreach ($this->filters as $filter => $value) {
                switch ($filter) {
                    case 'id':
                        ...
                        break;
                    case 'url_old':
                        $ands .= '  AND lg.`url_old` LIKE "%'.$value.'%" ';
                        break;
                    case 'url_new':
        ...
        if (isset($this->filters['error']) && $this->filters['error'] == 1) {
            $sql .= 'RIGHT JOIN ('."\n".
                ...
                (isset($this->filters['url_old']) && $this->filters['url_old'] != '')?
                    '   WHERE a.`url_old` LIKE "%'.$this->filters['url_old'].'%" '."\n":
                    ''
                ).
        ...
        if (!isset($this->filters['error']) || (isset($this->filters['error']) && $this->filters['error'] != 1)) {
            ...
            $sql = 'SELECT a.`url_old`, COUNT(a.`url_old`) AS "error_checkduplicate" '."\n".
                '   FROM `'._DB_PREFIX_.'lgseoredirect` a'."\n".
                '   WHERE `url_old` IN ("'.implode('", "', $ids).'")'."\n".
                '   GROUP BY a.`url_old`, a.`id_shop` '."\n";
            $duplicate_count = Db::getInstance()->ExecuteS($sql, true, false);
            $dc_final = array();
            foreach ($duplicate_count as $dc) {
                $dc_final[$dc['url_old']] = $dc['error_checkduplicate'];
            }
    ...
    public function getPagesNotFound($limit = 0, $offset = 100)
    {
        ...
        $this->sql_count = (int)$db->getValue('SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'pagenotfound`', false);
