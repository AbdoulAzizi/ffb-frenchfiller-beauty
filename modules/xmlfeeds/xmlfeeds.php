...
class Xmlfeeds extends Module
{
    ...
    public function feedsSettings($page)
    {
        ...
        $link = $xmlFeedUrl->get('id='.$page.$pass_in_link.'&affiliate=affiliate_name');
    ...
    public function printBlock($block_name = false, $info = array(), $only_checkbox = false)
    {
        ...
        foreach ($info as $y => $i) {
            ...
            $fields[$y]['value'] = !empty($this->tags_info[$i['name'].'+'.$i['table']]) ? $this->tags_info[$i['name'].'+'.$i['table']] : (!empty($i['placeholder']) ? '' : $i['title']);
    ...
    public function productsXmlSettings($page_id = false)
    {
        ...
        $html .= $this->printBlock(
            'Prices, Tax',
            array(
                array('name' => 'price', 'title' => 'base_price', 'table' => 'product'),
                array('name' => 'price_sale_blmod', 'title' => 'sale_price', 'table' => 'bl_extra'),
                array('name' => 'price_wt_discount_blmod', 'title' => 'price_without_discount', 'table' => 'bl_extra'),
                array('name' => 'price_shipping_blmod', 'title' => 'shipping_price', 'table' => 'bl_extra'),
                array('name' => 'wholesale_price', 'title' => 'wholesale_price', 'table' => 'product'),
                array('name' => 'only_discount_blmod', 'title' => 'only_discount_value', 'table' => 'bl_extra'),
    ...
    protected function validateFeedSettingsUpdate($POST)
    {
        if ((!empty($POST['update_feeds_s']) || !empty($POST['settings_cat']) || !empty($POST['settings_prod'])) && !empty($POST['feeds_name'])) {
            ...
            if (!empty($dateFrom) && !Validate::isDate($dateFrom)) {
                $errors[] = $this->l('Filter by date "from" value must be date (example: 2020-02-30)');
            }

            if (!empty($dateTo) && !Validate::isDate($dateTo)) {
                $errors[] = $this->l('Filter by date "to" value must be date (example: 2020-02-30)');
            }
