...
class ProductXmlApi
{
    ...
    public function getFeed(
        ...
        if (!empty($l)) {
            ...
            $xml_l = Db::getInstance()->ExecuteS('
                SELECT '._DB_PREFIX_.'product_lang.id_product, '._DB_PREFIX_.'product_lang.description_short AS description_short_blmod, 
            ...
            if (!empty($xml_l) && !empty($field)) {
                foreach ($xml_l as $xll) {
                    ...
                    if ($mode == 'h') {
                        $lang_prefix = $this->getLanguageCodeLong(ltrim($lang_prefix, '-'));
                    }
                    ...
                    foreach ($xll as $idl => $vall) {
                        ...
                        if ($mode == 'pub') {
                            $xml_lf[$id_cat . $l_iso] .= '<attribute><code>'.$xml_name_l[$idl].'</code><value><![CDATA[' . $vall . ']]></value></attribute>';
                        } else {
        ...
        foreach ($xml_d as $xdd) {
            ...
            $this->productParam['sale_price'][$xdd['pro_id']] = $this->getPriceFormat($salePrice);
            ...
            $xmlProduct = '<'.$block_name['cat-block-name'].$catBlockParam.(($mode == 'tt' || $mode == 'ep' || $mode == 'ro') ? ' id="'.REPLACE_COMBINATION.'id_product'.'"' : '').(($mode == 'ep' || $mode == 'ro') ? ' available="true"' : '').'>';
            ...
            if (!empty($this->productFeatures[$settings['vivino_bottle_size']]) && !empty($this->productFeatures[$settings['vivino_lot_size']])) {
                $xmlProduct .= '<bottles size="'.$this->productFeatures[$settings['vivino_bottle_size']].'">'.$this->productFeatures[$settings['vivino_lot_size']].'</bottles>';
            }
            ...
            if ($mode == 'pub') {
                $xmlProduct .= '<product-id>'.(!empty($combinationDefault['reference']) ? $combinationDefault['reference'] : '').'</product-id>';
                $xmlProductMru .= '<attribute><code>'.$xml_name[$id].'</code><value>'.$this->settings['pref_s'].$val.$this->settings['pref_e'].'</value></attribute>';
            }
            ...
            if (!empty($extra_field)) {
                ...
                foreach ($extra_field as $b_e) {
                    ...
                    if ($b_e['name'] == 'price_shipping_blmod') {
                        ...
                        $extraTag .= $this->getProductTax($product_class->id_tax_rules_group);
                ...
                if (!empty($features)) {
                    ...
                    if ($mode == 'i') {
                        ...
                        foreach ($features as $f) {
                            ...
                            $xmlProduct .= '<PARAM><NAME>'.$fieldFeature[$f['id_feature']]['title_xml'].'</NAME><VAL>'.$this->settings['pref_s'].$f['value'].$this->settings['pref_e'].'</VAL></PARAM>';
                        }
                    } elseif ($mode == 'ceo') {
                        foreach ($features as $f) {
                            if (empty($fieldFeature[$f['id_feature']])) {
            ...
            if (!empty($affiliate)) {
                $affiliate_prices = Db::getInstance()->ExecuteS('SELECT `affiliate_name`, `affiliate_formula`, `xml_name`
                    ...
                    WHERE `affiliate_name` = "'.pSQL($affiliate).'"
            ...
            if (!empty($combinations)) {
                ...
                $attributesByParentGroup = !empty($permissions['merge_attributes_by_group']) ? $mergeAttributesByGroup->getByParentGroup($productAttributes, $onlyInStock, $fieldGroupedAttributes) : array();
                ...
                foreach ($combinations as $c) {
                    ...
                    if ($mode == 'pub') {
                        $xml .= $this->replaceCombination($xmlProduct, $c, $xmlImages, $link_class, $product_class, $id_lang, $mode, $isAvailableWhenOutOfStock, $availabilityName, array(), $paramsCeneo);
                        $this->publicGrProducts = true;
                        $xmlProductMru .= $this->replaceCombination($xmlProductMruBeforeCombination, $c, $xmlImages, $link_class, $product_class, $id_lang, $mode, $isAvailableWhenOutOfStock, $availabilityName, $combinationImages, $paramsCeneo);
                        ...
                        $xml .= $this->replaceCombination($xmlProduct, $c, $xmlImages, $link_class, $product_class, $id_lang, $mode, $isAvailableWhenOutOfStock, $availabilityName, $combinationImages, $paramsCeneo);
                        $xml .= $this->getProductAttributeBranch($extra_attributes, $attributesList, $block_name, $one_branch, $c['id_product_attribute']);
                    }
                    ...
                    if (empty($permissions['merge_attributes_by_group']) || empty($attributesByParentGroup)) {
                        ...
                        $xml .= $this->getProductGroupedAttributeBranchByParentGroup($fieldGroupedAttributes, $attributesByParentGroup, $parentGroupName, $mergeAttributesByGroup->getParentGroup());
                    }
                ...
                $xml .= $this->fieldSorting($xmlProduct.'</'.$block_name['cat-block-name'].'>', $mode);
                $xmlProductMru .= '</product>';
            }
        ...
        if ($mode == 'pub') {
            if ($permissions['xml_type'] == 'products') {
                ...
                $xml = '<products>'.$xmlProductMruAll.'</products>'.$xml;
            }
        }
    ...
    public function replaceCombination(
        ...
        $xml = str_replace(REPLACE_COMBINATION.'wholesale_price', $this->getPriceFormat($feedPrice->getEditedPrice($combination['wholesale_price'], 'product_wholesale_price', $this->settings)), $xml);
    ...
    public function replaceCombinationToEmpty($xml, $combination, $images, $product_class, $mode, $paramsCeneo, $affiliatePriceFinal)
    {
        ...
        $xml = str_replace(REPLACE_COMBINATION.'url', $url, $xml);
        $xml = str_replace(REPLACE_COMBINATION.'wholesale_price', $this->getPriceFormat($feedPrice->getEditedPrice($product_class->wholesale_price, 'product_wholesale_price', $this->settings)), $xml);
        $xml = str_replace(REPLACE_COMBINATION.'sale_blmod', $priceSale, $xml);
            ...
            foreach ($fieldGroupedAttributes as $ag) {
                ...
                if ($mode == 'gla' || $mode == 'u' || $mode == 'mal' || $mode == 'naj' || $mode == 'zbo' || $mode == 'tov') {
                    ...
                    foreach ($valueList as $v) {
                        $xmlProduct .= '<PARAM><'.$paramName.'>'.$ag['title_xml'].'</'.$paramName.'><VAL>'.$this->settings['pref_s'].$v.$this->settings['pref_e'].'</VAL></PARAM>';
                    }
                } elseif ($mode == 'ceo') {
    ...
    public function getProductCategories($productId, $langId = false, $defaultCatId = 0, $returnId = false)
    {
        ...
        if (!empty($defaultCatId) && $this->isExistsCategoryGetAllParents) {
            ...
            foreach ($categoryDefault->getAllParents($langId) as $category) {
    ...
    public function fieldSorting($xml, $mode)
    {
        if ($mode == 'vi') {
