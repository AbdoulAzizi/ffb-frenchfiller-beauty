...
if (!empty($argv[1])) {
    ...
    if (class_exists('Context', false)) {
        ...
        $context->currency = (int)Configuration::get('PS_CURRENCY_DEFAULT');
    ...
    if ($now > $cache_period) {
        ...
        if (file_exists($file_url)) {
            Db::getInstance()->Execute('
				...
				VALUES
				("'.(int)$id.'", "'.(int)$part.'", "'.pSQL($permissions['file_name'].$affiliate_name).'", "'.pSQL($now).'", "'.pSQL($affiliate).'")
			');
        }
    }
}
...
function insert_statistics($feed_id = false, $affiliate = false)
{
    ...
    Db::getInstance()->Execute('
		...
		VALUES
		("'.(int)$feed_id.'", "'.pSQL($affiliate).'", "'.pSQL(date('Y-m-d H:i:s')).'", "'.pSQL(get_ip()).'")
	');
