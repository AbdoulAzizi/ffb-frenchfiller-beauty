...
"http://www.chronopost.fr/transport-express/livraison-colis/accueil/produits-tarifs/expertise-sectorielle/e-commerce/plateformes">Chronopost.fr
        ...
        if (Tools::strlen(json_decode(Configuration::get('CHRONOPOST_GENERAL_ACCOUNTS'),1)[0]['account'] < 8)) {
...
website to download a comptible version.';
    ...
    public function install()
    {
        ...
        Configuration::updateValue('CHRONOPOST_GENERAL_ACCOUNTS','[{"account":"19869502","subaccount":"","password":"255562","accountname":"Chronopost test"}]');
        ...
        Configuration::updateValue('CHRONOPOST_RDV_STATE1','0');
        Configuration::updateValue('CHRONOPOST_RDV_STATE2','0');
        Configuration::updateValue('CHRONOPOST_RDV_STATE3','0');
        Configuration::updateValue('CHRONOPOST_RDV_STATE4','0');
        ...
        foreach (self::$carriers_definitions as $product_code => $product) {
            Configuration::updateValue('CHRONOPOST_'.$product_code. '_ACCOUNT','19869502');
            Configuration::updateValue('CHRONOPOST_'.$product_code. '_ID','-1');
        }
    ...
    public static function trackingStatus($id_order, $shipping_number)
    {
        ...
        $subject = $module_instance->l('Tracking number for your order');
    ...
    protected function assignChronorelaisSmartyVars($params)
    {
    ...
    public function hookDisplayAfterCarrier($params)
    {
        ...
        $this->assignChronorelaisSmartyVars($params);
    ...
    public static function isReturnAvailable($order)
    {
        ...
        if (in_array($country->iso_code, $whitelist)) {
            return true;
        }
        return false;
    }
    ...
    public function hookAdminOrder($params)
    {
        ...
        if (count($LTHistory) > 0) {
            ...
            $accountUsedResults = self::getAccountTrackingNumber($params['id_order'], $LTHistory[0]);
        ...
        $chronopost_errors = array();
        if (isset($_SESSION['chronopost_errors'])) {
            $chronopost_errors = $_SESSION['chronopost_errors'];
            unset($_SESSION['chronopost_errors']);
        }

        $wsHelper = self::getWsHelper();
        $availContracts = $wsHelper->getContractsForMethod(self::$carriers_definitions[self::getCodeFromCarrier($carrier->id_reference)]['product_code']);

        $this->context->smarty->assign(
            array(
                'default_weight' => $cart->getTotalWeight(),
    ...
    private function carrierForm($code)
    {
        ...
        $availContracts = $wsHelper->getContractsForMethod(self::$carriers_definitions[$code]['product_code']);
    ...
    private function postProcess()
    {
        ...
        if (Tools::getValue('createnewcarrier')=="") {
            ...
            $default_address->city = $params['shipper']['city'];
            $default_address->postcode = $params['shipper']['zipcode'];
            $default_address->id_country = $country;
            $accounts_infos = array();
            $accounts = array();
            foreach ($params as $prefix => $var) {
                foreach ($var as $varname => $value) {
                   if(($varname == "account" && $prefix == "general") || $varname == "subaccount" || $varname == "password" || $varname == "accountname") {
                       $i = 0;
                       ...
                       if ($varname == 'account') {
                           if (!isset($accounts_infos[$value]['methods'])) {
                               $contract_infos = Chronopost::getAccountInformationByAccountNumber($value);
                               $accounts_infos[$value]['methods'] = $wsHelper->getMethodsForContract($contract_infos['account']);
                           }
                           // Check contract before saving
                           if ($var['id'] != '-1' && !in_array(self::$carriers_definitions[$prefix]['product_code'], $accounts_infos[$value]['methods'])) {
                               $this->_errors[] = Module::displayError(sprintf($this->l('Carrier %s is not available for the selected contract'), $prefix));
                               continue;
                           }
                       }
                ...
                if(!empty($accounts) && $prefix == 'general') {
                    ...
                    Configuration::updateValue('CHRONOPOST_'.Tools::strtoupper($prefix).'_ACCOUNTS', json_encode($accounts));
                }
            }
        }
        return true;
    ...
    public static function setWsShippingNumber($order, $shipping_number)
    {
        if (version_compare(_PS_VERSION_, '1.6.0.9', '>=')) {
            return $order->setWsShippingNumber($shipping_number);
        } else {
    ...
    {
        foreach (self::$carriers_definitions as $product_code => $data) {
            if($idReference == Configuration::get('CHRONOPOST_'.$product_code.'_ID')) {
                return $product_code;
            }
        }
        return false;
