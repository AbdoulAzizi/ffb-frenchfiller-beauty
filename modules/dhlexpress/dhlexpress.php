...
class Dhlexpress extends CarrierModule
{
     ...
     * @return array
     */
    public function checkAccount($mode, $login, $password)
    {
        if (!$login || !$password) {
            return array(
                'error' => true,
        ...
        if ($response && $response instanceof DhlQuoteResponse) {
            ...
            if (empty($errors)) {
                return array(
                    'error' => false,
                    'message' => $this->l('Your credentials has been checked successfully.')
                );
            } else {
                return array(
                    'error' => true,
            ...
            return array(
                'error' => true,
                'message' => $this->l('Cannot connect to DHL API.')
            );
        }
    }
     ...
     *
     */
    public function ajaxProcessCheckCredentials()
    {
        require_once(dirname(__FILE__).'/api/loader.php');
        require_once(dirname(__FILE__).'/classes/DhlTools.php');
        require_once(dirname(__FILE__).'/classes/logger/loader.php');
        ...
        $mode = Tools::getValue('mode') == 'prod' ? 1 : 0;

        $return = $this->checkAccount($mode, $login, $password);
