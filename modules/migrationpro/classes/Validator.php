...
class Validator
{
    private $fieldsForSkipping = ['nleft', 'nright', 'level_depth', 'id_shop_default'];
    ...
    private function validateField($field, $fieldData, $object)
    {
        ...
        if (isset($fieldData['size'])) {
            ...
            if ($length < $size['min'] || $length > $size['max']) {
                $object->{$field} = Tools::substr($size['min'], $size['max'], $object->{$field});
                $editedByTheModule = true;
                ...
                if (Tools::strtolower($fieldData['validate']) == 'iscleanhtml') {
                    ...
                    if ($fieldWithLanguage) {
                        ...
                        foreach ($object->{$field} as $langId => $value) {
                            if (!empty($object->{$field}) && !call_user_func(array('Validate', $fieldData['validate']), $object->{$field}[$langId])) {
